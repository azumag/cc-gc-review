...
# test.sh - cc-gen-reviewのテストスクリプト
...
test_help() {
    ...
    if $SCRIPT_DIR/cc-gen-review.sh -h | grep -q "Usage:"; then
        test_pass
    else
...
test_no_args() {
    ...
    if $SCRIPT_DIR/cc-gen-review.sh 2>&1 | grep -q "SESSION_NAME is required"; then
        test_pass
    else
...
test_tmux_session_creation() {
    ...
    timeout 3s $SCRIPT_DIR/cc-gen-review.sh -v --tmp-dir "$TEST_TMP_DIR" "$TEST_SESSION" &
    PID=$!
    sleep 1
...
test_file_watch() {
    ...
    $SCRIPT_DIR/cc-gen-review.sh -v --tmp-dir "$TEST_TMP_DIR" "$TEST_SESSION" &
    PID=$!
    sleep 2
...
test_tmp_dir_fallback() {
    ...
    timeout 3s $SCRIPT_DIR/cc-gen-review.sh -v "$TEST_SESSION" 2>&1 | grep -q "Using /tmp as temporary directory" &
    PID=$!
    sleep 1
