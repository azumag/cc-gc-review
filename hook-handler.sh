...
log() {
	local message
	message="[$(date +'%Y-%m-%d %H:%M:%S')] [hook-handler] $*"
	...
	if command -v gemini >/dev/null 2>&1; then
		...
		while [[ $content_count -lt $content_wait ]]; do
			if [[ -s "$temp_stdout" ]] || [[ -s "$temp_stderr" ]]; then
				...
				current_content=$(cat "$temp_stdout" 2>/dev/null)
				local filtered_content
				filtered_content=$(echo "$current_content" | grep -v "^\[dotenv@.*\] injecting env" | sed '/^$/d')
		...
		if [[ $gemini_exit_code -eq 124 ]]; then
			...
			if [[ "$error_output" =~ "status 429" ]] ||
				...
				# 生の出力をログに記録（デバッグ用）
				local raw_output
				raw_output=$(cat "$temp_stdout" 2>/dev/null)
		...
		if [[ "$is_rate_limit" == "true" ]]; then
			...
			if [[ $gemini_exit_code -eq 124 ]]; then
				...
				if [[ -z "$review_result" ]]; then
					...
					local raw_flash_output
					raw_flash_output=$(cat "$temp_stdout" 2>/dev/null)
					log "Raw flash stdout content: $raw_flash_output"
					log "Skipping review due to empty Flash content"
					return 1 # レビューファイルを作成しない
				else
					local flash_end_time
					flash_end_time=$(date +%s)
